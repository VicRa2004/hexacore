# Guía de Testing y Prevención de Errores (Hexacore)

Esta guía establece el estándar sobre cómo estructurar, diseñar y ejecutar las pruebas para la API del proyecto Hexacore. El objetivo no es solo lograr buena cobertura, sino enfocarse en **prevenir errores comunes**, probar los casos límite (`edge cases`) y verificar la arquitectura hexagonal mediante tests ágiles.

Utilizamos **`bun test`** (que viene integrado con Bun) como test runner y **`supertest`** para pruebas End-to-End en las capas de red, reemplazando a Jest para aprovechar la inmensa velocidad de Bun.
...
**Debes usar *mocks* de las dependencias inyectadas con TSyringe.**
...
describe("CreateUserUseCase", () => {
  ...
  it("debería lanzar UserAlreadyExistsError si el correo ya existe", async () => {
    ...
    mockUserRepository.findByEmail.mockResolvedValue({ id: "123", email: "test@test.com" });
  ...
  it("debería registrar el usuario correctamente y devolver Dto", async () => {
    ...
    const result = await useCase.run({ email: "new@test.com", password: "123", roleId: "USER" });
...
describe("E2E Rutas de Usuario (/api/users)", () => {
  ...
  it("debería crear un nuevo usuario y retornarle código 201", async () => {
    const p = { email: "jhon@doe.com", password: "PasswordFuerte123", roleId: "BASIC" };
